...
export default async function handler(req, res) {
  ...
  if (pinErr || !landlord || !landlord.id) return res.status(401).json({ ok: false, error: "Incorrect PIN" })
  ...
  if (linkErr || !token_hash) return res.status(500).json({ ok: false, error: "could not start session" })
